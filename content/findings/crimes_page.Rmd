...
description: "Trends in criminal justice indicators"
weight: 2
draft: false
...
rates_by_gender_system %>%
  ...
  labs(color = "Gender", title = "Prison Incarceration Rate", y = "Incarcerated Individuals per 1,000\n", x = "\nYear")  +
  facet_wrap(gender~.) + theme(legend.position = "none")
...
rates_by_gender_system %>%
  ...
  gghighlight(county_name == "Halifax County", use_direct_label = FALSE, calculate_per_facet = TRUE, unhighlighted_params = list(alpha = 0.2)) +
  scale_y_continuous(limits = c(0, scale_max)) +
  labs(color = "Gender", title = "Jail Incarceration Rate", subtitle = "", y = "Incarcerated Individuals per 1,000\n", x = "\nYear")  +
  facet_wrap(gender~.) +
  theme(legend.position = "none")
```
<br>

As the incarcerated population has increased, so too has the cost per inmate, producing an unsustainable burden on Halifax County's budget. In 2017, it cost more than \$20,000 in corrections expenses for every jailed inmate in the county, which is similar to Virginia-wide costs reported by the Vera Institute of Justice.[^VERA2] 

In general, calculating costs per inmate may be misleading, because expenses can appear to increase as a result of a decrease in incarceration. However, similar patterns emerge when calculating costs per Halifax resident, and raw corrections expenses have also been increasing in recent years.

The high costs of incarceration suggest that a Family and Consumer Science Agent position could quickly provide a positive return on investment even if it prevented relatively few encounters with the criminal justice system amongst vulnerable residents in Halifax County.
...
On average, 2,081 crimes came to the attention of law enforcement within the 10-year period between 2010 and 2019 in Halifax County. Across the years, the number of crimes was relatively stable, with slightly higher levels of observed crime before 2013 and slightly lower levels after 2013. 
...
The following figure displays the types of crimes that came to the attention of law enforcement in Halifax County from most common to least common on the left and their corresponding clearance by arrest rate on the right. Drug violations, simple assault, larceny, and destruction of property made up the bulk of the crimes; however, this did not necessarily correspond to higher arrest rates. While drug violations and simple assaults resulted in arrests nearly 50% of the time, larceny and destruction of property resulted in arrests much less frequently. This disparity highlights the complex process that proceeds a crime coming to the attention of law enforcement and precedes an arrest - namely that there exist many latent factors, including difficulty of "solving" the crime and effort put forth by law enforcement, that impact whether an arrest is made. 
...
#### Crime Trends by Race (per 1,000 individuals)

The figure below highlights the disparity between the number of crimes by Black and White people that came to attention of law enforcement across Virginia counties. Overall, the number of crimes by White people that law enforcement were alerted to was below 50 per 100,000 White people for nearly all counties in Virginia. In contrast, there were many counties with over 50 crimes per 100,000 Black people, including several at or above 200 crimes per 100,000 Black people. In Halifax County specifically (denoted by the lines in color), the number of crimes by Black people that law enforcement were alerted to is nearly twice that of those by White people. This disparity again highlights the complex process by which crimes are given law enforcement attention - namely that any historical policies or varying levels of law enforcement presence in neighorhoods will impact said attention.
...
The following figures compare the average number of crimes per 1000 people law enforcement was alerted to across all Virginia counties from 2010 to 2019. The darker colors indicate more crimes per 1000 people whereas the lighter colors indicate less crimes per 1000 people.  

#### Map of All Crimes

Halifax County, compared to other Virginia counties, was around the median across all of Virginia in total crimes per 1000 that law enforcement were alerted to. The same conclusion holds when examining the average number of crimes per 1000 people by race. 
...
Comparing just drug related crimes across Virginia counties, Halifax County was again close to the median levels of drug crimes per 1000 that law enforcement are alerted to across all of Virginia. The same conclusion holds when examining the average number of drug related crimes by race. 
